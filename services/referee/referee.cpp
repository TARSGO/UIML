...
static struct _Referee {
    ...
    struct {
        ...
        uint16_t bytesReceived; // （收到不完整帧时）已经收到的字节数。定义包含帧头、CmdId、data、帧尾CRC16等。
        uint16_t dataLength;; // （收到不完整帧时）帧头指定的data段字节数，如果已收字节数不足帧头，此字段未定义
    } recvFsm;
} Referee;
...
void Referee_Init(ConfItem* conf)
{
    ...
    Referee.recvPacketBufSize = 64; // 接收缓冲区长度由于除了车间通信没有什么很大变化的可能，所以暂时没有放进配置文件
