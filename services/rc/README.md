...
- **广播右摇杆数据**：`/rc/right-stick`
  ...
  - **广播类型**：普通方式（映射表数据帧）
    ...
    | 数据字段名 | 数据类型  |            说明            |
    | :--------: | :-------: | :------------------------: |
    |    `x`     | `int16_t` | x 方向通道值(范围-660~660) |
    |    `y`     | `int16_t` | y 方向通道值(范围-660~660) |
...
- **广播拨杆数据**：`/rc/switch`
  ...
  - **广播类型**：普通方式（映射表数据帧）
    ...
    - **数据帧格式**
      ...
      | `right` | `uint8_t` | 左拨杆位置(上 1/中 3/下 2)(每次广播仅有其中一个数据) |
...
- **广播鼠标移动数据**：`/rc/mouse-move`
  ...
  - **广播类型**：普通方式（映射表数据帧）
    ...
    | 数据字段名 | 数据类型  |        说明         |
    | :--------: | :-------: | :-----------------: |
    |    `x`     | `int16_t` | 鼠标 x 方向移动速度 |
    |    `y`     | `int16_t` | 鼠标 y 方向移动速度 |
...
- `RC_InitKeyJudgeTime(rc,0x3ffff,50,500)`默认全部按键 50~500ms 之间为短按，大于 500ms 为长按，可自行修改
- 若要移植到非 HAL 库平台，需要将`RC_UpdateKeys()`中的`HAL_GetTick()`修改为目标平台中用于获取时间戳（既系统运行时间，单位 ms）的 API 函数
