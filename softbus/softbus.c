    ...
    Vector_ForEach(hashList, hashNode, HashNode) // 遍历所有hash节点
    {
        if (hash == hashNode->hash)
        {
            Vector_ForEach(hashNode->receiverNodes,
                           receiverNode,
                           ReceiverNode) // 遍历该hash节点下所有receiver
            {
...
void _Bus_PublishTopic(const char *name, SoftBusFrame *frame)
{
    ...
    Vector_ForEach(hashList, hashNode, HashNode) // 遍历所有hash节点
    {
        if (hash == hashNode->hash) // 匹配到hash值
        {
            Vector_ForEach(hashNode->receiverNodes,
                           receiverNode,
                           ReceiverNode) // 遍历改hash节点的所有receiver
            {
                if (strcmp(name, receiverNode->name) ==
                    0) // 匹配到receiver，抛出该receiver所有回调函数
                {
    ...
    Vector_ForEach(hashList, hashNode, HashNode) // 遍历所有hash节点
    {
        if (hash == hashNode->hash)
        {
            Vector_ForEach(hashNode->remoteNodes,
                           remoteNode,
                           RemoteNode) // 遍历该hash节点下所有remote
            {
                if (strcmp(name, remoteNode->name) == 0) // 匹配到已有remote注册回调函数
                {
                    if (remoteNode->callbackNode.callback ==
                        Bus_EmptyRemoteFunction) // 如果该remote下为空回调函数
                    {
                        remoteNode->callbackNode =
                            ((CallbackNode){bindData, callback}); // 更新回调函数
                        ret = 0;
                        goto Unwrap;
                    }
                    ret = -3; // 该服务已注册过服务器，不允许一个服务有多个服务器
                    goto Unwrap;
                }
            }
            CallbackNode callbackNode = {
